...
import torchvision
from inat_dataset import iNaturalist
...
def main_dataset_worker(gpu, ngpus_per_node, args, dataset_i=0):
    ...
    if model_name != '':
        ...
        for k in list(state_dict.keys()):
            ...
            if k.startswith('model.encoder_q'
                            ) and not k.startswith('model.encoder_q.fc'):
                state_dict[k[len("encoder_q."):]] = state_dict[k]

            if k.startswith('model.encoder'
                            ) and not k.startswith('model.encoder.fc'):
    ...
    if dataset_i == 'imnet':
      traindir = os.path.join('/data/datasets/ImageNet','train')
      valdir = os.path.join('/data/datasets/ImageNet','val')
    ...
    #    trainfname,
     #   transforms.Compose([
      ...
      val_dataset = torchvision.datasets.ImageFolder(
          ...
          train_dataset = iNaturalist(root='/data/datasets/iNat2021', mode="train")
          val_dataset = iNaturalist(root='/data/datasets/iNat2021', mode="val")
    if args.environment.distributed:
