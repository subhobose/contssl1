    ...
    if args.environment.distributed:
        ...
        if args.environment.gpu is not None:
            ...
            model = torch.nn.parallel.DistributedDataParallel(
                model, device_ids=[args.environment.gpu],broadcast_buffers=False )
        else:
            model.cuda()
...
def train(train_loader,
    ...
    cosLoss = nn.CosineSimilarity(dim=0)
    ...
    for data in train_loader:
        ...
        #print(p1.size())
        #cosi = cosLoss(p1[0], p2[0])
        
        #for each image, print ++ and min of all +-(max of all cosine distances)
        ...
        for i in range(256):
          allNeg = []
          anglepos = cosLoss(p1[i].detach().cpu(), p2[i].detach().cpu())
          maxD = 0
          for j in range(256):
            if j==i:
              continue
            angleposneg = cosLoss(p1[i].detach().cpu(), p2[j].detach().cpu())
            allNeg.append(angleposneg)
            if angleposneg > maxD:
              maxD = angleposneg
          
          #print("For image {}, ++ = {}, +-(min) = {}".format(i+1, math.acos(anglepos)*180/3.14, math.acos(maxD)*180/3.14))
          allNeg = np.array(allNeg)
          allNegAvg = np.mean(allNeg)
          allNegArr.append(math.acos(allNegAvg)*180/3.14)
          posArr.append(math.acos(anglepos)*180/3.14)
          posNegArr.append(math.acos(maxD)*180/3.14)
        
        posArr = np.array(posArr)
        meanPositivePerBatch = np.mean(posArr)
        print("Mean pos {}".format(meanPositivePerBatch))
        meanPos.append(meanPositivePerBatch)
        posNegArr = np.array(posNegArr)
        meanPosNegPerBatch = np.mean(posNegArr)
        print("Mean posneg {}".format(meanPosNegPerBatch))
        meanPosNeg.append(meanPosNegPerBatch)
        allNegArr = np.array(allNegArr)
        meanNegPerBatch = np.mean(allNegArr)
        print("Mean neg {}".format(meanNegPerBatch))
        meanNeg.append(meanNegPerBatch)
        
        
              
        #print("Cosine for positive example {}".format(cosi))
        #angleVector = torch.acos(cosi)*180/3.14
        #for i in range(256):
            #print("For image {} angle is - {}".format(i+1,angleVector[i]))
        #meanAnglePerBatch = torch.mean(angleVector)
        #writer.add_scalar("Batchwise Average Angle", meanAnglePerBatch, batchNum)
    ...
    imgArr = [i+1 for i in range(batchNum-1)]
    print(len(imgArr))
    print(len(meanPos))
    print(len(meanPosNeg))
    print(len(meanNeg))
    plt.plot(imgArr, meanPos, label = 'positive examples')
    plt.plot(imgArr, meanPosNeg, label = 'min of negative examples')
    plt.plot(imgArr, meanNeg, label = 'negative examples')
    plt.legend()
    plt.show()