...
def main_worker(gpu, ngpus_per_node, args):
    ...
    main_dataset_worker(gpu, ngpus_per_node, args)


def main_dataset_worker(gpu, ngpus_per_node, args, dataset_i='imnet'):
    best_acc1 = 0
    ...
    if model_name != '':
        ...
        for k in list(state_dict.keys()):
            ...
            if k.startswith('encoder_q'
                            ) and not k.startswith('encoder_q.fc'):
                state_dict[k[len("encoder_q."):]] = state_dict[k]

            if k.startswith('encoder'
                            ) and not k.startswith('encoder.fc'):
    ...
    traindir = os.path.join('/home/subhrangsu/contssl/continuous_ssl_problem/cifar100/cifar100','train')
    valdir = os.path.join('/home/subhrangsu/contssl/continuous_ssl_problem/cifar100/cifar100','test')
    ...
    val_dataset = torchvision.datasets.ImageFolder(
        valdir,
        transforms.Compose([
            transforms.Resize(int(args.lincls[dataset_i].data.insize * 256 /
                        float(224))),
...
def train(train_loader, model, criterion, optimizer, epoch, args, writer=None):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        # compute output
        output = model(images)
        loss = criterion(output, target) #batchwise loss
        #print("Batch Number {}".format(batchNum))
        writer.add_scalar("Loss/Batch CIFAR100 Train", loss, batchNum)
        batchNum = batchNum+1
...
def validate_with_stats(val_loader,
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(val_loader):
            ...
            writer.add_scalar("Loss/Batch CIFAR100 Validn", loss, batchNum)
            batchNum = batchNum+1
