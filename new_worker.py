...
def train(train_loader,
    ...
    for data in train_loader:
        ...
        for i in range(100):
          anglepos = cosLoss(z1[i].detach().cpu(), z2[i].detach().cpu())
          maxD = 0
          for j in range(100):
            if j==i:
              continue
            angleposneg = max(cosLoss(z1[i].detach().cpu(), z2[j].detach().cpu()), cosLoss(z1[i].detach().cpu(), z1[j].detach().cpu()), cosLoss(z2[i].detach().cpu(), z1[j].detach().cpu()), cosLoss(z2[i].detach().cpu(), z2[j].detach().cpu()))
            if angleposneg > maxD:
              maxD = angleposneg
          
          print("For image {}, ++ = {}, +-(min angle/max cosine) = {}".format(i+1, anglepos, maxD))
